...
agesex_lphu %>%
  ...
  #
  f_graph_pyramid(prop_variable = aboriginal_prop,
                  fill_variable = lphu_short,
                  fill_values   = c("NEPHU" = colour_darkblue,
                                    "VIC"   = colour_dodgerblue))
```
...
agesex_lphu %>% 
  ...
  #
  f_graph_pyramid(prop_variable = prop,
...
for (i in nephu_lga) {
  ...
  dplyr::mutate(
    ...
    hover_text = paste0("\nSex: ", sex,
                        ...
                        "\nPercentage: ", sprintf("%.1f", abs(prop)))) %>% 
  #
  f_graph_pyramid(chart_title   = paste0(i, " LGA"),
                  prop_variable = prop,
                  fill_variable = aboriginal_status,
                  fill_values   = c("Identified as Aboriginal\nand/or Torres Strait Islander"       = colour_lightblue,
                                    "Did not identify as Aboriginal\nand/or Torres Strait Islander" = colour_burgundy))
...
f_graph_bar_simple_combine(figure_number, figure_percent)
```
...
#write.csv(table_lga, paste0(here::here(), "/data", "/table_5a1", ".csv"), row.names = FALSE)
